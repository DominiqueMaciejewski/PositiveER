...
# Reference: Weerdemeijer et al. https://osf.io/rcy8t?view_only=0d70ea4e0b8d4241901516131cc38cad 

## Compliance  ----------------------------------------

# Calculate percentage compliance
PER_data$compliance <- PER_data$Measures_valid/ 70 * 100 #here i divide by 60 (total number of beeps) and *100 for nice natural number of compliance

### Main dataset ----------------------------------------

# Main dataset had 33% valid assessments as inclusion criteria (pre-registered)

#### Comparing included and excluded participants ----------------------------------------

data0 <- PER_data %>% 
  filter(compliance >= 33) 

### Subdatasets ----------------------------------------

# For now I only create datasets with different exclusion criteria based on varying compliance (0%-50%, with 5% increases)
# Compliance is based on 70 assessments (14 days * 5 assessments; note that some participants had more)

# !!More needs to be added in consultation with Egon & other co-authors!!

# Subset datasets 
for (i in 1:11){ #for loop that creates novel dataframes based on different compliance exclusion values
  assign(paste0("data",i), subset(PER_data, compliance>=((i-1)*5)))
}

## Save datasets (RData) ----------------------------------------

### RData
# Generate the data frame names using a numeric sequence
dataframe_names <- paste0("data", 0:11)

# Loop through each data frame and save them
for (name in dataframe_names) {
  # Define the file path and name for saving
  file_path <- file.path(here("data"), paste0(name, ".RData"))
...
# # Commented out now, because most Mplus files were fun on the supercomputer
# 
...
# 
# # We made two changes to the model specifications:
# # 1) The models 1b and 2b for dampening did not converge with fbiterations
# # = .005 for some of the convergence criteria.
# # Those models I kept separate, using the Mplus default, for which the models
# # did converge (fbiterations = .05)
# 
# # 2) The model 1b/2b for co-dampening for the compliance criterion on 5%&15% threshold compliance
# # for inclusion did not converge with fbiterations = .005 (data2 & data4). Here, I used the Mplus default
# # (fbiterations = .05)
# 
...
# createModels(here("Scripts","Templates","Template_1b_damp.txt"))
# createModels(here("Scripts","Templates","Template_1b_codamp.txt"))
# 
...
# createModels(here("Scripts","Templates","Template_2b_damp.txt"))
# createModels(here("Scripts","Templates","Template_2b_contdep_damp.txt"))
# createModels(here("Scripts","Templates","Template_2b_codamp.txt"))
# createModels(here("Scripts","Templates","Template_2b_contdep_codamp.txt"))
# 
...
round_columns <- function(df) {
  ...
  df_transformed <- df %>%
    mutate_at(vars(columns_to_round), ~ round(., 2))
...
PER_data %>% 
  ...
  dplyr::summarise(n=n_distinct(Eth_ID))
...
hist_variables <- c("PA_enth","PA_tevr","PA_ener","PA_kalm",
                    "PA_daadk","PA_vrol","PA_dankb",
                    "NA_irri","NA_verv","NA_nerv","NA_verdr",
                    "NA_boos","NA_somb",
                    "PER_att", "PER_refl", "PER_self", "PER_expr", 
                    "PER_damp", "PER_codamp", "PER_capit")

for (var in hist_variables) {
  hist<- ggplot(data0, aes(x=get(var))) + geom_histogram(bins=101,color="black", fill="white")
  print(hist)
}
...
## save table as word file
flextable::save_as_docx(table_7, path = here("Tables", "table_7.docx"))
